...
func createXEPGDatabase() (err error) {
	...
	var generateHashForChannel = func(ItemTitle string, M3UFile string, m3uID string, groupTitle string, tvgID string, tvgName string, uuidKey string, uuidValue string) string {
		hash := md5.Sum([]byte(ItemTitle + M3UFile + m3uID + groupTitle + tvgID + tvgName + uuidKey + uuidValue))
	...
	for _, v := range Data.XEPG.Channels {
		...
		channelHash := generateHashForChannel(channel.Name, channel.FileM3UName, channel.FileM3UID, channel.GroupTitle, channel.TvgID, channel.TvgName, channel.UUIDKey, channel.UUIDValue)
	...
	for _, dsa := range Data.Streams.Active {
		...
		m3uChannelHash := generateHashForChannel(m3uChannel.Name, m3uChannel.FileM3UName, m3uChannel.FileM3UID, m3uChannel.GroupTitle, m3uChannel.TvgID, m3uChannel.TvgName, m3uChannel.UUIDKey, m3uChannel.UUIDValue)
